<?php
	...
	$sql_delete_responses = "DELETE FROM responses WHERE quiz_id = " . $_GET['quiz_id'] . ";";
	$results_delete_responses = $mysqli->query($sql_delete_responses);
	if (!$results_delete_responses) {
		echo $mysqli->error;
		exit();
	}

	$sql_delete_quiz = "DELETE FROM quizzes WHERE quiz_id = " . $_GET['quiz_id'] . ";";
	$results_delete_quiz = $mysqli->query($sql_delete_quiz);
	if (!$results_delete_quiz) {
		echo $mysqli->error;
		exit();
	}
